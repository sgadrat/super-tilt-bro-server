...
void DatagramDispatcher::run() {
	while (true) {
		...
		if (destination_queue) {
			try {
				syslog(LOG_DEBUG, "DatagramDispatcher: pushing message from %s:%d to its destination queue", message->sender.address().to_string().c_str(), message->sender.port());
			...
			try {
				syslog(LOG_DEBUG, "DatagramDispatcher: pushing message from %s:%d to default queue", message->sender.address().to_string().c_str(), message->sender.port());
