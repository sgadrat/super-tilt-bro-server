...
std::vector<uint8_t> generate_state(uint8_t stage, uint8_t character_1, uint8_t character_2)
{
	// Initialize emulator
...
int main(int argc, char** argv) {
	...
	uint8_t stage = 0;
	{
		args::Args params = args::parse(argc, argv, {"--character-a", "--character-b", "--stage"});
	...
	std::vector<uint8_t> state = generate_state(stage, character_1, character_2);
